...
// const RANDOM_MUTATIONS = 4
// const MAX_DESCENDANTS_TO_EXPLORE = 500
console.log("geneticSolver.js Loaded");
...
function geneticSolver(groups, ofSize, forRounds, forbiddenPairs=[], discouragedGroups=[], onProgress, g, m, d) {
  ...
  const GENERATIONS = g;
  const RANDOM_MUTATIONS = m;
  const MAX_DESCENDANTS_TO_EXPLORE = d;
...
function forEachPair(array, callback) {
  ...
  //console.log("forEachPair(GS): " & array[i]);
}

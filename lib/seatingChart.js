...
const State = Record({
  finishedRounds: List(),
  // Array of tables, where each table is an array of indices of players grouped so far.
  groups: List(),
  nextPlayer: 0,
  totalCost: 0,
  weights: null,
...
module.exports = function seatingChart({
  ...
  function exploreState(state) {
    ...
    // Check existing groups for possible placements
    const childStates = []
    ...
    // Add possibility of placing in a new group if there are any left
